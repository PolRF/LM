...
def from_pretrained_rope_gpt2(
    device: torch.device,
) -> tuple[GPTLM, ModelConfig]:
    ...
    my_model_keys = [
        k for k in my_model_keys if not k.endswith(".attn.bias")
...
def remove_prefix(state_dict):
    return {
        k.replace("module._orig_mod.", ""): v for k, v in state_dict.items()
    }


def resume_from_checkpoints(
    config, model_config: ModelConfig
) -> tuple[GPTLM, int, float]:
